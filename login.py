...
def get_redir_url():
    try:
        resp = session.get(
            "http://connect.rom.miui.com/generate_204", allow_redirects=False)
...
def do_login(redir_url, sa_token):
    ...
    oauth_location = "https://api.215123.cn/ac/auth/oauthRedirect?" + \
        oauth_resp.headers["location"].split(
            "?")[1] + "&serviceName=chinaTelecom"
